        ...
        for (; it_first != maze_scheme.first.end(); it_first++) {
            ...
            if (*it_first) {
                 painter.drawLine(start_pos_x, start_pos_y, start_pos_x, start_pos_y+step_y);
                  //qDebug() << start_pos_x << ":" << start_pos_y << ":" << start_pos_x << ":" << start_pos_y+step_y;
            }
            count++;
         ...
         if(!cave_scheme.first.empty()) {
            ...
            for (; it_first != cave_scheme.first.end(); it_first++) {
                if (*it_first) {
                  ...
                  //   qDebug() << start_pos_x << ":" << start_pos_y << ":" << start_pos_x + step_x << ":" << start_pos_y + step_y;
...
void MainWindow::on_generate_maze_btn_clicked()
{
    ...
    tester.show_vector("Down borders after maze generate",
                       maze_scheme.second, maze_scheme.size_col, maze_scheme.size_rows);
    start_ball.is = false;
    finish_ball.is = false;
...
void MainWindow::on_find_way_btn_clicked()
{
    if (start_ball.is && finish_ball.is) {
        ...
        finder.point_array_creation(&maze_scheme, start_ball, finish_ball, &array_lines);
    } else {
...
{
     // Кнопка инициализации лабиринта
    init_cave();
    repaint();
