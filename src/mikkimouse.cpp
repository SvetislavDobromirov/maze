...
std::vector<int> MikkiMouse::start_searching(model *maze, baloon start_ball, baloon finish_ball) {
    ...
    std::cout << "Col = " << finish_ball.col << " Row << " << finish_ball.row << std::endl;
    ...
    while(1) {
       ...
       if (disizion == -1) {
            std::cout<< "Нашел" << std::endl;
            //result_way[current_point.first, current_point.second] = track;
            break;
        }
    ...
    tester.show_vector("После действия", result_way,
                      maze->size_col, maze->size_rows);

    tester.show_vector("Линии", maze->line_way,
                      maze->size_col, maze->size_rows);
...
int MikkiMouse::find_way(model *maze) {
    ...
    std::cout << "CURRENT POS: COL: " << current_point.first << " ROW: " << current_point.second << std::endl;
...
int MikkiMouse::line_way_pos(model *maze, int pos_temp, int pos_current) {
    std::cout << "Pos temp: " << result_way[pos_temp] << " Pos Current " << result_way[pos_current] << std::endl;
    if (result_way[pos_temp] == 0) {
        maze->line_way[pos_current] = 1;
...
void MikkiMouse::point_array_creation(model * maze, baloon start_ball, baloon finish_ball,
  ...
  std::cout  << col << " " << row << std::endl;
          ...
          if (maze->first[pos_current] != 1) {// Если нет границы справа тогда логично проверить клетку справа
              if (maze->line_way[pos_current + 1]) {
                      ...
                      std::cout  << point.x_start << " : " << point.y_start <<  " : " <<
                                    point.y_finish << " : " << point.x_finish << std::endl;
